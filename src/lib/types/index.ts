...
export interface GpoDetail extends GpoSummary {
    ...
    content: { policies: GpoPolicy[] };
    metadata: { author: string; tags: string[] };
    targeting: {
        mode:      'include' | 'exclude' | 'all';
        groups:    string[];
        machines:  string[];
        os_filter?: { distribution?: string; version_min?: string };
    };
    ...
    version_history: { version: number; updated_at: string; updated_by: string }[];
    rollback: { enabled: boolean; automatic_on_failure: boolean; keep_backups: number };
}
...
export interface Settings {
    general:  { domain: string; organization: string };
    agent:    { default_poll_interval: number; max_poll_interval: number; secret_rotation_days: number; log_batch_size: number };
    security: { jwt_expiration_seconds: number; mfa_enabled: boolean; password_min_length: number; max_login_attempts: number; lockout_duration_minutes: number };
    ldap:     { base_dn: string; uid_start: number; gid_start: number };
    logs:     { retention_audit_days: number; retention_security_days: number; retention_gpo_days: number; retention_system_days: number; retention_debug_days: number };
}